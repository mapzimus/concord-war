<script>
  ...
  /** @type {{ specs: import('../data/manifest.js').LayerSpec[], installations?: any[] }} */
  let { specs = [], installations = [] } = $props();
  ...
  // Match the clicked feature to a workflow-authored installation (for blurb + stats + source).
  let match = $derived.by(() => {
    if (!f || !f.properties || !f.properties.name || !installations.length) return null;
    const fn = norm(f.properties.name);
    if (!fn) return null;
    return (
      installations.find((i) => {
        const ino = norm(i.name);
        return ino && (ino.includes(fn) || fn.includes(ino));
      }) || null
    );
  });

  const SKIP = new Set(['name', 'side', 'note', 'blurb', 'OBJECTID', 'id', 'geometry', 'confidence', 'kind', 'category', 'source_url']);
...
{#if f}
  <aside class="drawer" aria-label="Feature details">
    ...
    <h3>{match?.name ?? f.properties.name ?? spec?.label ?? 'Feature'}</h3>
    <div class="meta">
      {#if spec}<span class="layer">{spec.label}</span>{/if}
      {#if (match?.side ?? f.properties.side)}
        {@const side = match?.side ?? f.properties.side}
        <span class="badge badge-{String(side).toLowerCase()}">{side}</span>
      {/if}
    </div>

    {#if match?.blurb}<p class="note">{match.blurb}</p>
    {:else if f.properties.blurb}<p class="note">{f.properties.blurb}</p>
    {:else if f.properties.note}<p class="note">{f.properties.note}</p>{/if}

    {#if match?.stats?.length}
      <dl>
        {#each match.stats as s}
          <div class="row"><dt>{s.label}</dt><dd>{s.value}</dd></div>
        {/each}
      </dl>
    {:else if rows(f.properties).length}
      <dl>
    ...
    {#if match?.source_url || spec?.source_url}
      <p class="src">Source: {match?.source_url ?? spec?.source_url}</p>
    {/if}
  </aside>
{/if}
...
<style>
  ...
  h3 { font-family: var(--font-display); font-size: 1.3rem; margin: 0 24px 6px 0; }
  .meta { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; margin-bottom: 8px; }
  .layer { font-size: 0.82rem; color: #6b5a2a; }
  .badge { font-size: 0.72rem; font-weight: 700; padding: 2px 8px; border-radius: 2px; border: 1px solid var(--ink); }
